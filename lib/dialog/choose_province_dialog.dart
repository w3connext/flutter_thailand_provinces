...
class _ChooseProvinceDialogState extends State<ChooseProvinceDialog> {
  List<ProvinceDao>? _listProvincesFilter;
  TextEditingController _searchProvinceController = TextEditingController();

  @override
  void initState() {
    _listProvincesFilter = List.of(widget.listProvinces);
    super.initState();
  }
...
//          decoration: BoxDecoration(borderRadius: BorderRadius.circular(16)),
  ...
  Widget buildListView() {
    return Stack(
      children: <Widget>[
        ListView.builder(
          itemCount: _listProvincesFilter?.length ?? 0,
          itemBuilder: (context, index) {
            return buildRowProvince(_listProvincesFilter![index]);
          },
        ),
        Center(
          child: Visibility(
            visible: _listProvincesFilter?.isEmpty ?? false,
            child: Text(
              "ไม่มีข้อมูล",
      ...
      child: Column(
        children: <Widget>[
          Container(
            ...
            decoration: BoxDecoration(
              color:
                  widget.colorBackgroundSearch ?? Colors.white.withOpacity(0.7),
            ...
            child: TextField(
              ...
              onChanged: (text) async {
                final list = widget.listProvinces.where(
                  (item) {
